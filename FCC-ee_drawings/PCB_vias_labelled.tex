...
\clip (0.24*\whv, -4*\h-\HT-\HS-\HP) rectangle (0.5615*\whv + 0.01, 4*\h+\HT+\HS+\HP);
  % LAYER -6
  \fill\HV ({-0.5 * \whv}, {{2.5*\h+\HT+\HS+\HP}}) rectangle ++ (\whv, \h) ;
  % LAYER -5
  \fill\iso ({-0.5 * \whv}, {{2.5*\h+\HT+\HS}}) rectangle ++ (\whv, \HP) ;
  \draw[- stealth]({0.5 * \whv}, {{3*\h + \HT+\HS+\HP}}) -- ({0.51 * \whv}, {{2.5*\h + \HT+\HS+\HP}}) node[pos=1, right] {High voltage};
...
%    \draw ({0.5 * \whv}, {1.5*\h + \HT}) --  ({0.52 * \whv}, {{1.5*\h + \HT}});
    \draw [- stealth] ({0.5 * \wp}, {{2*\h + \HT+\HS}}) -- ({0.51 * \whv}, {{1.5*\h + \HT+\HS}}) node[pos=1, right] {Signal pick-up};
%    \dimline[color=black, extension start length= 0,  extension end length= 0,
%      extension start style={line width = 2pt},extension end style={line width = 2pt},
...
%            {$\mathbfit{h_m}$};
  ...
  \fill\trace ({0.4 * \whv - \viaSafety - \via}, {{-0.5*\h}}) rectangle ++ (\via, 2.5*\h + \HT + \HS) ;
      ...
      \draw  [- stealth]  ({0.5 * \whv}, {{1*\h + \HT}}) -- ({0.51 * \whv}, {{1*\h + \HT}}) node[pos=1, right] {Ground shield};
       \draw  [- stealth]  ({0.5 * \whv}, {{0.*\h }}) -- ({0.51 * \whv}, {{0.*\h }}) node[pos=1, right] {Signal extraction};

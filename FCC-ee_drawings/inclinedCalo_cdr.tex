...
\begin{tikzpicture}[xscale=2,yscale=2,spy using outlines={rectangle, magnification=8, size=7cm, height=6cm, connect spies}]\footnotesize
    \spy [ultra thick, black] on (43.65,{3.4-4}) in node [fill=white] at (24.1,{1.9-2}); % Since xscale=2, spy coordinate have to be *2...
  ...
  \begin{scope}[black, line width = {\cell}]
    \foreach \i in {0,..., \shownumplanes}
    {
      ...
      \foreach \j in {0,..., 6}
      {
        ...
        %Draw the separation in longitudinal readout layers
...
%%%%%    \node [below of = planeLabel] {\Huge{ $\Delta\varphi=$\num[round-mode=places,round-precision=4]{\dPhiTowerRadians}}};
    ...
    \draw[double arrow=4pt colored by white and violet!80] (22.2,2.2) -- (22.2+1.31, 1.4); %towards mangification box
    \node[red!80] at (22.2+1.7,2.4) {\huge absorber};
    \draw[double arrow=4pt colored by white and red!80] (22.2+1.7,2.2) -- (24.5, 1.4);
    \node[green!80] at (22.2+1.7+1.4,2.4) {\huge readout};
    \draw[double arrow=4pt colored by white and green!80] (22.2+1.7+1.4,2.2) -- (22.2+1.7+1.55, 1.4);

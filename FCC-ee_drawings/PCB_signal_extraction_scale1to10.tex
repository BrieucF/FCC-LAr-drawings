...
% still would like to have a scaling that is fixed to some values to ease the interpretation
\begin{document}
\def\H{0.12}
\def\HP{0.003}
\def\HT{0.017}
...
%\def\Rshift{0.}
%\def\Rin{216.}
\def\drFirst{2.322267}  % length of presampler parallel to it in cm
\def\drRest{5.385130} % length of other layers parallel to them
\def\deltaThetaFirst{0.57}
\def\overlapTracePad{0.2}  % length of presampler parallel to it in cm
\begin{tikzpicture}[xscale=10,yscale=1,line/.style={<->,shorten >=0.4cm,shorten <=0.4cm},thick]\Large
%\begin{tikzpicture}[xscale=12,yscale=0.6,line/.style={<->,shorten >=0.4cm,shorten <=0.4cm},thick]\Large
    ...
    \pgfmathsetmacro{\widthInner}{216.*tan(\deltaThetaFirst)*1}
    \message{\widthInner}
    \pgfmathsetmacro{\widthOuter}{(216+\drFirst+11*\drRest)*tan(\deltaThetaFirst)*1}
    \message{\widthOuter}
    \clip (-0.4, -0.4) rectangle ({\widthOuter+0.2},{\drFirst+11*\drRest+3});
  ...
  \foreach \iLayer in {11,...,0} { %Draw the trapeze with FR4
    ...
    \pgfmathsetmacro{\Lmin}{\Rmin*tan(\deltaThetaFirst)*1}
    \pgfmathsetmacro{\Lmax}{\Rmax*tan(\deltaThetaFirst)*1}
    \message{\Rmin}
    \message{\Rmax}
    ...
    % }{
    %   \pgfmathsetmacro{\wsbetween}{ (0.95*(216+2+6*9)*tan(\deltaThetaFirst)/10 - 10 * \ws - 2 * \margin * 0.95*(216+2+6*9)*tan(\deltaThetaFirst) / ((10 + 1)*10) }
    % }
    ...
    \node[scale=1.75,blue] at (-0.*\Lmax+0.08, {\Rmax-\Rshift+2*(\Rmax-\Rmin)/10 +0.5}) {\Huge ${\theta=90^{\circ}}$};
    \node[scale=1.75,blue] at (1*\Lmax-0.15, {\Rmax-\Rshift+2*(\Rmax-\Rmin)/10 +0.5}) {\Huge ${\theta=89.43^{\circ}}$};
    }{}
  }
      ...
      \ifthenelse{\lengthtest{\iLayer pt=0 pt}}{
        ...
        \pgfmathsetmacro{\upstep}{\Lmax/(4)} %Ntrace + 1
          \foreach \iTrace in {1,...,4} { %-0.5*\downstep is to acount for the spacing to the neighborhood cell
            \fill\trace ({ -0.5*\downstep + \iTrace * \downstep - 0.5 * \wt}, {\Rout-\Rshift}) -- ({ -0.5*\downstep + \iTrace * \downstep + 0.5 * \wt}, {\Rout-\Rshift}) -- ({ -0.5*\upstep + \iTrace * \upstep + 0.5 * \wt}, {\Rmax - \Rshift +\overlapTracePad}) -- ({ -0.5*\upstep + \iTrace * \upstep - 0.5 * \wt}, {\Rmax - \Rshift + \overlapTracePad} ) -- cycle ;
          }
          \fill\trace ({ 2*\downstep - 0.5 * \wt}, {\Rout-\Rshift}) -- ({  2*\downstep  + 0.5 * \wt}, {\Rout-\Rshift}) -- ({ 2*\downstep + 0.5 * \wt}, {\Rmax - \Rshift - \drFirst + \overlapTracePad}) -- ({ 2*\downstep - 0.5 * \wt}, {\Rmax - \Rshift - \drFirst + \overlapTracePad}) -- cycle ;
          
      }{}
      % second cell
      \pgfmathsetmacro{\LminFirstLayer}{216.*tan(\deltaThetaFirst)*1}
      \ifthenelse{\lengthtest{\iLayer pt=1 pt}}{
        \fill\trace ({\LminFirstLayer/4 - 0.5 * \wt}, {\Rout-\Rshift}) -- ({ \LminFirstLayer/4 + 0.5 * \wt}, {\Rout-\Rshift}) -- ({\Lmax/4 + 0.5 * \wt}, {\Rmax - \Rshift + \overlapTracePad}) -- ({ \Lmax/4 - 0.5 * \wt}, {\Rmax - \Rshift + \overlapTracePad} ) -- cycle ;
      }{}
      %third cell
      \ifthenelse{\lengthtest{\iLayer pt=2 pt}}{
        \fill\trace({3*\LminFirstLayer/4 - 0.5 * \wt}, {\Rout-\Rshift}) -- ({ 3*\LminFirstLayer/4 + 0.5 * \wt}, {\Rout-\Rshift}) -- ({3*\Lmax/4 + 0.5 * \wt}, {\Rmax - \Rshift + \overlapTracePad}) -- ({ 3*\Lmax/4 - 0.5 * \wt}, {\Rmax - \Rshift + \overlapTracePad} ) -- cycle ;
      }{}
      % all other cells
      \ifthenelse{\lengthtest{\iLayer pt>\NlayerExtractedfront pt}}{
        \pgfmathsetmacro{\downstep}{\Lmin/(8)}
        \pgfmathsetmacro{\upstep}{\widthAtTraceExit/(8)}
        \fill\trace ({\Lmin + 0.5*\downstep + (\NlayerExtractedfront - \iLayer)*\downstep - 0.5 * \wt}, {\Rmin - \Rshift - \overlapTracePad}) -- ({\Lmin + 0.5*\downstep + (\NlayerExtractedfront - \iLayer)*\downstep + 0.5 * \wt}, {\Rmin - \Rshift - \overlapTracePad}) -- ({\widthAtTraceExit + 0.5*\upstep + (\NlayerExtractedfront - \iLayer)*\upstep + 0.5 * \wt}, {\Rout - \Rshift}) -- ({\widthAtTraceExit + 0.5*\upstep + (\NlayerExtractedfront - \iLayer)*\upstep - 0.5 * \wt}, {\Rout - \Rshift} ) -- cycle ;
      }{}
